...
while (<FILE>)
{
	...
	elsif ($rpt == 8)
	{
		...
		$sysp_access{$grantee}->{$privilege}->{through_role} = "$adm|$def";
	}
}
...
# $sysp_access{$grantee}->{$privilege}->{through_user} = 1;
		# $sysp_access{$grantee}->{$privilege}->{through_role} = 0;
		# $sysp_access{$grantee}->{$privilege}->{through_role}->{admin} = $adm;
		
		
#output object privileges
open OUT, ">$out_dir\\" . $db . "_object_privs.txt" or die "Can't open the output file: $!\n";
print OUT "Database|Grantee|Object|Type|Privilege|Through?|Through Role|Grantor|Grantable|Role Admin|Role Default\n";
for my $grantee (sort keys %objp_access)
{
	for my $obj_name ( sort keys %{ $objp_access{$grantee} } )
	{	
		for my $privilege ( sort keys %{ $objp_access{$grantee}->{$obj_name} } )
		{	
			if ( $objp_access{$grantee}->{$obj_name}->{$privilege}->{through_role} eq "no" )
			{
				#through user
				print OUT "$db|$grantee|$obj_name|";
				print OUT "$objects{$obj_name}->{type}|";
				print OUT "$privilege|through_user|n/a|";
				my($grantor,$grantable) = split/\|/, $objp_access{$grantee}->{$obj_name}->{$privilege}->{through_user};
				unless ($grantor) { $grantor = "-"; }
				unless ($grantable) { $grantable = "-"; }
				print OUT "$grantor|$grantable|";
				print OUT "n/a|n/a\n";
			
			}
			elsif ( $objp_access{$grantee}->{$obj_name}->{$privilege}->{through_user} eq "no" )
			{
				#through role
				print OUT "$db|$grantee|$obj_name|";
				print OUT "$objects{$obj_name}->{type}|";
				print OUT "$privilege|through_role|";
				my($role,$adm,$def) = split/\|/, $objp_access{$grantee}->{$obj_name}->{$privilege}->{through_role};
				unless ($role) { $role = "-"; }
				unless ($adm) { $adm = "-"; }
				unless ($def) { $def = "-"; }
				print OUT "$role|n/a|n/a|$adm|$def\n";
			}
		}
	}
}
